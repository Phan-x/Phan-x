    ...
    <ScreenContainer className="px-5" edges={["top", "left", "right"]}>
      <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={styles.content}>
        ...
        <Card style={styles.assetCard}>
          <View style={styles.assetTop}><Text style={styles.cardLabel}>إجمالي الأصول</Text><Pressable onPress={() => setHidden(!hidden)} style={({ pressed }) => [pressed && styles.pressed]}><MaterialIcons name={hidden ? "visibility-off" : "visibility"} size={20} color={PHANX.white}/></Pressable></View>
          <Text style={styles.assetValue}>{hidden ? "••••••" : total}</Text>
          <View style={styles.assetBottom}><View style={styles.changeBadge}><MaterialIcons name="trending-up" size={15} color={PHANX.white}/><Text style={styles.changeText}>متاح</Text></View><Text style={styles.assetSub}>رصيد USDT</Text></View>
          <View style={styles.assetGlowOne}/><View style={styles.assetGlowTwo}/>
        </Card>

        <View style={styles.quickGrid}>{quickActions.map((action) => <Pressable key={action.label} onPress={() => router.push(action.route as never)} style={({ pressed }) => [styles.quickItem, pressed && styles.pressed]}><View style={styles.quickIcon}><MaterialIcons name={action.icon} size={21} color={PHANX.green}/></View><Text style={styles.quickLabel}>{action.label}</Text></Pressable>)}</View>

        <Pressable onPress={() => router.push("/settings")} style={({ pressed }) => [styles.promo, pressed && styles.pressed]}><View style={styles.promoIcon}><MaterialIcons name="bolt" size={24} color={PHANX.gold}/></View><View style={styles.promoCopy}><Text style={styles.promoTitle}>أكمل إعداد محفظتك</Text><Text style={styles.promoSubtitle}>فعّل المصادقة الثنائية لتحصل على حماية أعلى</Text></View><MaterialIcons name="chevron-left" size={22} color={PHANX.ink}/></Pressable>

        <SectionTitle title="ابدأ مع Phan-x" action="عرض الكل" onAction={() => router.push("/menu")} />
        <View style={styles.startRow}><Card style={styles.startCard} onPress={() => router.push("/receive")}><View style={[styles.startIcon, { backgroundColor: "#E9F7EF" }]}><MaterialIcons name="call-received" size={21} color={PHANX.green}/></View><Text style={styles.startTitle}>استقبل عملة</Text><Text style={styles.startSub}>أضف أول أصل لمحفظتك</Text></Card><Card style={styles.startCard} onPress={() => router.push("/trade")}><View style={[styles.startIcon, { backgroundColor: "#FFF3DD" }]}><MaterialIcons name="swap-horizontal-circle" size={21} color={PHANX.gold}/></View><Text style={styles.startTitle}>جرّب التبديل</Text><Text style={styles.startSub}>بدّل أصولك بسهولة</Text></Card></View>
...
const styles = StyleSheet.create({
  ...
  assetCard: { backgroundColor: PHANX.green, borderColor: PHANX.green, minHeight: 158, overflow: "hidden", marginBottom: 20 },
  ...
  assetValue: { color: PHANX.white, fontSize: 34, fontWeight: "900", letterSpacing: -1, marginTop: 14, textAlign: "right" },
  ...
  changeText: { color: PHANX.white, fontWeight: "800", fontSize: 12 }, assetSub: { color: "#B4E2C9", fontSize: 12, fontWeight: "600" },
  ...
  quickItem: { alignItems: "center", width: "19%" }, quickIcon: { width: 44, height: 44, borderRadius: 15, backgroundColor: PHANX.greenSoft, justifyContent: "center", alignItems: "center", marginBottom: 7 }, quickLabel: { color: PHANX.ink, fontSize: 11, fontWeight: "700", textAlign: "center" },
  promo: { flexDirection: "row", alignItems: "center", gap: 11, backgroundColor: "#FFF9EC", padding: 13, borderRadius: 17, marginBottom: 24, borderWidth: 1, borderColor: "#F8E6BA" }, promoIcon: { width: 40, height: 40, borderRadius: 13, backgroundColor: "#FFF1CC", justifyContent: "center", alignItems: "center" }, promoCopy: { flex: 1 }, promoTitle: { color: PHANX.ink, fontSize: 13, fontWeight: "800", textAlign: "right" }, promoSubtitle: { color: PHANX.muted, fontSize: 11, marginTop: 4, textAlign: "right" },
  startRow: { flexDirection: "row", gap: 10, marginBottom: 25 }, startCard: { flex: 1, borderRadius: 18, padding: 14 }, startIcon: { width: 38, height: 38, borderRadius: 12, justifyContent: "center", alignItems: "center", marginBottom: 12 }, startTitle: { fontSize: 13, color: PHANX.ink, fontWeight: "800", textAlign: "right" }, startSub: { color: PHANX.muted, fontSize: 10, marginTop: 5, lineHeight: 15, textAlign: "right" },
