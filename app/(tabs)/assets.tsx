...
import { Card, CoinMark, IconButton, SectionTitle } from "@/components/phanx-ui";
import { COINS, PHANX } from "@/constants/phanx";
...
export default function AssetsScreen() {
  ...
  return <ScreenContainer className="px-5" edges={["top", "left", "right"]}>
    ...
    <View style={styles.searchBox}><MaterialIcons name="search" size={20} color={PHANX.muted}/><TextInput value={query} onChangeText={setQuery} placeholder="ابحث عن عملة" placeholderTextColor="#9CA8A1" style={styles.searchInput} textAlign="right"/><Pressable onPress={() => setQuery("")}><MaterialIcons name="tune" size={19} color={PHANX.green}/></Pressable></View>
...
const styles = StyleSheet.create({ header: { flexDirection: "row", alignItems: "center", justifyContent: "space-between", paddingTop: 12, marginBottom: 18 }, kicker: { fontSize: 12, color: PHANX.muted, textAlign: "right", marginBottom: 3 }, title: { fontSize: 26, fontWeight: "900", color: PHANX.ink, textAlign: "right" }, actions: { flexDirection: "row", gap: 8 }, tabs: { flexDirection: "row-reverse", gap: 7, marginBottom: 16 }, tab: { paddingHorizontal: 11, paddingVertical: 8, borderRadius: 10, backgroundColor: PHANX.surface }, activeTab: { backgroundColor: PHANX.green }, tabText: { color: PHANX.muted, fontSize: 10, fontWeight: "700" }, activeTabText: { color: PHANX.white }, totalCard: { backgroundColor: "#F0F9F4", borderColor: "#D9F0E1", marginBottom: 21 }, label: { color: PHANX.muted, fontSize: 12, textAlign: "right" }, total: { color: PHANX.ink, fontSize: 30, fontWeight: "900", textAlign: "right", marginTop: 11 }, totalFoot: { flexDirection: "row", alignItems: "center", justifyContent: "space-between", marginTop: 8 }, subtle: { color: PHANX.muted, fontSize: 11 }, positivePill: { backgroundColor: PHANX.green, paddingHorizontal: 8, paddingVertical: 5, borderRadius: 8 }, positive: { color: PHANX.white, fontSize: 11, fontWeight: "800" }, searchBox: { flexDirection: "row", alignItems: "center", gap: 8, height: 46, borderWidth: 1, borderColor: PHANX.line, borderRadius: 14, paddingHorizontal: 12, marginBottom: 10 }, searchInput: { flex: 1, color: PHANX.ink, fontSize: 13 }, list: { paddingBottom: 25 }, coinRow: { flexDirection: "row", alignItems: "center", paddingVertical: 13, borderBottomWidth: 1, borderBottomColor: PHANX.line, gap: 11 }, coinName: { flex: 1 }, symbol: { color: PHANX.ink, fontSize: 14, fontWeight: "800", textAlign: "right" }, name: { color: PHANX.muted, fontSize: 10, marginTop: 4, textAlign: "right" }, coinValue: { alignItems: "flex-end" }, amount: { color: PHANX.ink, fontSize: 12, fontWeight: "700" }, value: { color: PHANX.muted, fontSize: 10, marginTop: 4 }, change: { width: 54, alignItems: "flex-end" }, changeText: { color: PHANX.green, fontSize: 11, fontWeight: "800" }, loss: { color: PHANX.red }, empty: { color: PHANX.muted, textAlign: "center", padding: 30 }, pressed: { opacity: 0.6 } });
