  ...
  return (
    <ScreenContainer className="px-5" edges={["top", "left", "right"]}>
      <ScrollView contentContainerStyle={styles.content}>
        ...
        {list.isLoading ? (
          <View style={styles.center}><ActivityIndicator color={PHANX.green} /></View>
        ) : items.length === 0 ? (
          <View style={styles.center}>
            <MaterialIcons name="notifications-none" size={40} color={PHANX.muted} />
          ...
          items.map((item: any) => (
            ...
            >
              <View style={[styles.icon, { backgroundColor: item.read ? PHANX.surface : PHANX.greenSoft }]}>
                <MaterialIcons name={item.userId ? "person" : "campaign"} size={20} color={item.read ? PHANX.muted : PHANX.green} />
              </View>
...
const styles = StyleSheet.create({
  ...
  markAll: { color: PHANX.green, fontSize: 10, fontWeight: "800" },
  unread: { flexDirection: "row", alignItems: "center", justifyContent: "flex-end", gap: 6, marginBottom: 11 },
  unreadDot: { width: 7, height: 7, borderRadius: 4, backgroundColor: PHANX.green },
  unreadText: { color: PHANX.muted, fontSize: 11 },
  center: { alignItems: "center", justifyContent: "center", paddingVertical: 60, gap: 10 },
  emptyText: { color: PHANX.muted, fontSize: 12 },
  row: { flexDirection: "row", alignItems: "flex-start", gap: 11, paddingVertical: 16, borderBottomWidth: 1, borderBottomColor: PHANX.line },
  icon: { width: 43, height: 43, borderRadius: 15, alignItems: "center", justifyContent: "center" },
  copy: { flex: 1 },
  rowTitle: { color: PHANX.ink, textAlign: "right", fontSize: 13, fontWeight: "900" },
  rowText: { color: PHANX.muted, textAlign: "right", fontSize: 11, lineHeight: 17, marginTop: 4 },
  time: { color: "#A1AAA5", textAlign: "right", fontSize: 9, marginTop: 6 },
  dot: { width: 8, height: 8, borderRadius: 4, backgroundColor: PHANX.green, marginTop: 6 },
  pressed: { opacity: 0.6 },
});
