...
const TIERS = [
  { level: 1, rate: "10%", color: PHANX.green, bg: PHANX.greenSoft, icon: "person" as const, label: "المستوى الأول", desc: "من دعوتهم مباشرة" },
  { level: 2, rate: "5%", color: PHANX.gold, bg: "#FBF2E1", icon: "people" as const, label: "المستوى الثاني", desc: "من دعاهم أصدقاؤك" },
  { level: 3, rate: "2.5%", color: PHANX.purple, bg: "#EEECFB", icon: "hub" as const, label: "المستوى الثالث", desc: "الدائرة الثالثة" },
];
...
export default function ReferralScreen() {
  ...
  const shareLink = async () => {
    const text = `انضم إلى Phan-x عبر كود الإحالة الخاص بي واحصل على مكافآت: ${referralCode}\n${referralLink}`;
    if (typeof navigator !== "undefined" && "share" in navigator) {
      try {
        await navigator.share({ title: "Phan-x", text, url: referralLink });
        return;
      } catch {
      ...
      <ScrollView contentContainerStyle={styles.content} showsVerticalScrollIndicator={false}>
        ...
        <View style={styles.hero}>
          ...
          <View style={styles.gift}>
            <MaterialIcons name="card-giftcard" size={30} color={PHANX.white} />
          </View>
        ...
        <View style={styles.code}>
          ...
          <Pressable onPress={copyCode} hitSlop={8}>
            <MaterialIcons name="content-copy" size={19} color={PHANX.green} />
          </Pressable>
        </View>
        <Pressable onPress={shareLink} style={({ pressed }) => [styles.button, pressed && styles.pressed]}>
          <MaterialIcons name="share" size={18} color={PHANX.white} />
            ...
            return (
              <View key={tier.level} style={[styles.tierCard, { backgroundColor: tier.bg }]}>
                <View style={[styles.tierIcon, { backgroundColor: tier.color }]}>
                  <MaterialIcons name={tier.icon} size={18} color={PHANX.white} />
                </View>
        ...
        <Card style={styles.infoCard}>
          <MaterialIcons name="info-outline" size={16} color={PHANX.muted} />
        ...
        <Card>
          {stats.isLoading ? (
            ...
            <View style={styles.emptyWrap}>
              <MaterialIcons name="inbox" size={26} color={PHANX.line} />
            ...
            stats.data.history.map((row: any, idx: number) => (
              <View key={row.id} style={[styles.rewardRow, idx > 0 && styles.rewardRowBorder]}>
                <View style={[styles.rewardIcon, { backgroundColor: TIERS[row.level - 1]?.bg ?? PHANX.greenSoft }]}>
                  <Text style={[styles.rewardLevel, { color: TIERS[row.level - 1]?.color ?? PHANX.green }]}>L{row.level}</Text>
                </View>
                <View style={{ flex: 1 }}>
...
const styles = StyleSheet.create({
  ...
  hero: { alignItems: "center", backgroundColor: PHANX.ink, borderRadius: 24, paddingVertical: 28, paddingHorizontal: 20, overflow: "hidden" },
  heroGlow: { position: "absolute", top: -60, right: -40, width: 180, height: 180, borderRadius: 90, backgroundColor: "rgba(11,135,84,0.35)" },
  gift: { width: 58, height: 58, borderRadius: 20, backgroundColor: PHANX.green, alignItems: "center", justifyContent: "center", marginBottom: 14 },
  heroTitle: { color: PHANX.white, fontSize: 19, fontWeight: "900", textAlign: "center" },
  ...
  heroStatValue: { color: PHANX.white, fontSize: 22, fontWeight: "900" },
  heroStatLabel: { color: "#AABBB1", fontSize: 10, marginTop: 5 },
  heroDivider: { width: 1, height: 34, backgroundColor: "rgba(255,255,255,0.15)" },

  label: { color: PHANX.ink, fontSize: 12, fontWeight: "800", textAlign: "right", marginTop: 24, marginBottom: 8 },
  code: { height: 52, borderWidth: 1.5, borderColor: PHANX.green, borderRadius: 14, flexDirection: "row", alignItems: "center", justifyContent: "space-between", paddingHorizontal: 16, backgroundColor: PHANX.greenSoft },
  codeText: { color: PHANX.ink, fontSize: 15, fontWeight: "900", letterSpacing: 1 },
  button: { height: 50, borderRadius: 14, backgroundColor: PHANX.green, flexDirection: "row", gap: 8, alignItems: "center", justifyContent: "center", marginTop: 11 },
  buttonText: { color: PHANX.white, fontSize: 13, fontWeight: "900" },

  section: { color: PHANX.muted, fontSize: 12, fontWeight: "800", textAlign: "right", marginTop: 26, marginBottom: 10 },
  ...
  tierLabel: { color: PHANX.ink, fontSize: 11, fontWeight: "800", marginTop: 4, textAlign: "center" },
  tierDesc: { color: PHANX.muted, fontSize: 9, marginTop: 2, textAlign: "center" },
  tierFooter: { marginTop: 10, alignItems: "center", borderTopWidth: 1, borderTopColor: "rgba(0,0,0,0.06)", paddingTop: 8, width: "100%" },
  tierCount: { color: PHANX.muted, fontSize: 9 },
  tierEarned: { fontSize: 11, fontWeight: "900", marginTop: 2 },

  infoCard: { flexDirection: "row-reverse", alignItems: "flex-start", gap: 8, marginTop: 16, backgroundColor: PHANX.surface },
  infoText: { flex: 1, color: PHANX.muted, fontSize: 10.5, lineHeight: 16, textAlign: "right" },

  emptyWrap: { alignItems: "center", justifyContent: "center", paddingVertical: 28, gap: 8 },
  emptyText: { color: PHANX.muted, fontSize: 11, textAlign: "center" },

  rewardRow: { flexDirection: "row", alignItems: "center", gap: 10, paddingVertical: 12 },
  rewardRowBorder: { borderTopWidth: 1, borderTopColor: PHANX.line },
  ...
  rewardName: { color: PHANX.ink, fontSize: 12, fontWeight: "800", textAlign: "right" },
  rewardMeta: { color: PHANX.muted, fontSize: 10, marginTop: 4, textAlign: "right" },
  rewardAmount: { color: PHANX.green, fontSize: 11, fontWeight: "900" },
