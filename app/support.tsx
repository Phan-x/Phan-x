 ...
 * publishing. TELEGRAM_USERNAME without the "@" (e.g. "Phan-xSupport"),
...
export default function SupportScreen() {
  ...
  const [chat, setChat] = useState<ChatMsg[]>([
    { id: "welcome", from: "bot", text: "مرحباً 👋 أنا مساعد Phan-x الآلي. اسألني عن أي شيء يخص التطبيق (الإيداع، السحب، كلمة المرور...) وسأجيبك فوراً." },
  ]);
    ...
    <ScreenContainer className="px-5" edges={["top", "left", "right"]}>
            ...
            <View style={[styles.bubble, m.from === "user" ? styles.bubbleUser : styles.bubbleBot]}>
              {m.from === "bot" && (
                <View style={styles.botTag}>
                  <MaterialIcons name="smart-toy" size={12} color={PHANX.green} />
            ...
            {m.contact && (
              <View style={styles.contactRow}>
                <Pressable onPress={openTelegram} style={({ pressed }) => [styles.contactBtn, styles.telegramBtn, pressed && styles.pressed]}>
                  <MaterialIcons name="send" size={16} color={PHANX.white} />
      ...
      <View style={styles.composer}>
        ...
        <Pressable onPress={handleSend} style={({ pressed }) => [styles.send, pressed && styles.pressed]}>
          <MaterialIcons name="send" size={18} color={PHANX.white} />
        </Pressable>
      </View>
    </ScreenContainer>
  );
...
const styles = StyleSheet.create({
  ...
  bubbleUser: { alignSelf: "flex-end", backgroundColor: PHANX.green, borderTopRightRadius: 4 },
  bubbleBot: { alignSelf: "flex-start", backgroundColor: PHANX.surface, borderTopLeftRadius: 4 },
  botTag: { flexDirection: "row-reverse", alignItems: "center", gap: 4, marginBottom: 5 },
  botTagText: { color: PHANX.green, fontSize: 9, fontWeight: "800" },
  bubbleText: { color: PHANX.ink, fontSize: 12, lineHeight: 20, textAlign: "right" },
  bubbleTextUser: { color: PHANX.white },
  sectionTitle: { color: PHANX.muted, fontSize: 11, fontWeight: "800", textAlign: "right", marginTop: 6, marginBottom: 10 },
  faqWrap: { flexDirection: "row", flexWrap: "wrap", gap: 8, justifyContent: "flex-end" },
  chip: { borderWidth: 1, borderColor: PHANX.line, borderRadius: 20, paddingHorizontal: 13, paddingVertical: 9 },
  chipText: { color: PHANX.ink, fontSize: 11, fontWeight: "700" },
  directContact: { marginTop: 26, borderTopWidth: 1, borderTopColor: PHANX.line, paddingTop: 16 },
  ...
  contactBtnText: { color: PHANX.white, fontSize: 12, fontWeight: "800" },
  composer: { borderTopWidth: 1, borderTopColor: PHANX.line, paddingTop: 10, paddingBottom: 10, flexDirection: "row", gap: 8 },
  input: { flex: 1, height: 44, borderWidth: 1, borderColor: PHANX.line, borderRadius: 14, paddingHorizontal: 13, color: PHANX.ink, fontSize: 12 },
  send: { width: 44, height: 44, borderRadius: 14, backgroundColor: PHANX.green, alignItems: "center", justifyContent: "center" },
