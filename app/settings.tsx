    ...
    <ScreenContainer className="px-5" edges={["top", "left", "right"]}>
      <ScrollView contentContainerStyle={styles.content}>
        ...
        <Card style={styles.banner}>
          <MaterialIcons name="security" size={23} color={PHANX.green} />
          ...
          {items.map((item, i) => (
            ...
            >
              <View style={styles.icon}>
                <MaterialIcons name={item.icon} size={18} color={PHANX.green} />
              </View>
              ...
              {item.key === "twoFactor" ? (
                <Switch
                  ...
                  thumbColor={twoFactor ? PHANX.green : "#fff"}
                />
              ) : (
        ...
        <Card>
          <Pressable onPress={() => router.push("/support")} style={({ pressed }) => [styles.row, pressed && styles.pressed]}>
            <View style={[styles.icon, { backgroundColor: "#FFF5DE" }]}>
              <MaterialIcons name="support-agent" size={18} color={PHANX.gold} />
            </View>
            <View style={styles.copy}>
          ...
          >
            <View style={[styles.icon, { backgroundColor: "#F2F3F2" }]}>
              <MaterialIcons name="description" size={18} color={PHANX.ink} />
            </View>
            <View style={styles.copy}>
...
const styles = StyleSheet.create({
  ...
  title: { color: PHANX.ink, fontSize: 20, fontWeight: "900" },
  banner: { flexDirection: "row", alignItems: "center", gap: 10, backgroundColor: "#F0F9F4", borderColor: "#D8F0E1", marginBottom: 22 },
  bannerCopy: { flex: 1 },
  bannerTitle: { color: PHANX.ink, fontSize: 12, fontWeight: "900", textAlign: "right" },
  bannerSub: { color: PHANX.muted, fontSize: 10, marginTop: 4, textAlign: "right" },
  enable: { backgroundColor: PHANX.green, borderRadius: 9, paddingHorizontal: 9, paddingVertical: 7 },
  enableText: { color: PHANX.white, fontSize: 10, fontWeight: "800" },
  section: { color: PHANX.muted, fontWeight: "800", fontSize: 12, textAlign: "right", marginBottom: 9, marginTop: 5 },
  row: { minHeight: 65, flexDirection: "row", alignItems: "center", gap: 10, borderBottomWidth: 1, borderBottomColor: PHANX.line },
  icon: { width: 35, height: 35, borderRadius: 12, backgroundColor: PHANX.greenSoft, alignItems: "center", justifyContent: "center" },
  copy: { flex: 1 },
  label: { color: PHANX.ink, fontSize: 12, fontWeight: "800", textAlign: "right" },
  sub: { color: PHANX.muted, fontSize: 10, marginTop: 4, textAlign: "right" },
